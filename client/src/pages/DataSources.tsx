...
const STATUS_COLORS: Record<string, string> = {
  ...
  not_collecting: 'bg-slate-800 text-slate-400 border-slate-700',
};
      ...
      <div className="flex-shrink-0 px-6 py-4 border-b border-slate-800 bg-slate-900/50">
        ...
        <div className="grid grid-cols-3 gap-3 mt-4">
          {[
            ...
            { label: 'Not Collecting', count: stats.not_collecting, color: 'text-slate-400' },
          ].map(s => (
            <div key={s.label} className="bg-slate-800/50 border border-slate-800 rounded-lg px-4 py-2.5">
        ...
        <div className="flex gap-3 mt-3">
          <input
            className="flex-1 bg-slate-800 border border-slate-700 rounded-lg px-3 py-1.5 text-slate-200 placeholder-slate-500 focus:outline-none focus:border-blue-500 text-sm"
            placeholder="Search data sources…"
            value={search}
            onChange={e => setSearch(e.target.value)}
          />
          <select
            className="bg-slate-800 border border-slate-700 rounded-lg px-3 py-1.5 text-slate-300 text-sm focus:outline-none focus:border-blue-500"
        ...
        {loading ? (
          ...
          <div className="space-y-1.5">
            {filtered.map(source => {
              ...
              return (
                <div key={source.id} className="bg-slate-900 border border-slate-800 rounded-xl overflow-hidden">
                  {/* Row */}
                  <div className="flex items-center gap-3 px-4 py-3">
                    ...
                    >
                      <svg className={`w-3.5 h-3.5 text-slate-500 flex-shrink-0 transition-transform ${isExpanded ? 'rotate-90' : ''}`}
                        fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" />
                      </svg>
                      <div className="min-w-0">
                        <div className="text-sm font-medium text-slate-200">{source.name}</div>
                        {source.description && <div className="text-xs text-slate-500 truncate mt-0.5">{source.description}</div>}
                      </div>
                    </button>

                    <span className="text-xs bg-slate-800 border border-slate-700 text-slate-400 px-2 py-0.5 rounded flex-shrink-0">
                      {source.category}
                    </span>
                    <span className="text-xs text-slate-500 w-20 text-center flex-shrink-0">
                  ...
                  {isEditingStatus && (
                    <div className="px-4 pb-4 border-t border-slate-800 pt-3 bg-slate-800/30">
                      <div className="text-xs font-medium text-slate-400 mb-3">Edit Collection Status</div>
                      <div className="grid grid-cols-3 gap-3">
                        <div>
                          <label className="text-xs text-slate-500 block mb-1">Status</label>
                          <select value={statusForm.status} onChange={e => setStatusForm(f => ({ ...f, status: e.target.value }))}
                            className="w-full px-2 py-1.5 bg-slate-800 border border-slate-700 rounded-lg text-xs text-slate-200 focus:outline-none focus:border-blue-500">
                            <option value="collecting">Collecting</option>
                            <option value="partial">Partial</option>
                            <option value="not_collecting">Not Collecting</option>
                          </select>
                        ...
                        <div>
                          <label className="text-xs text-slate-500 block mb-1">Collection Method</label>
                  ...
                  {isExpanded && (
                    <div className="border-t border-slate-800 bg-slate-950/40">
                      {loadingTech === source.id ? (
                        <div className="px-5 py-4 text-xs text-slate-500">Loading techniques…</div>
                      ) : (
                          ...
                          {techPickerOpen === source.id && (
                            <div className="mb-3 border border-slate-700 rounded-lg overflow-hidden bg-slate-900">
                              <input
                                ...
                                className="w-full px-3 py-2 bg-slate-800 text-xs text-slate-300 focus:outline-none border-b border-slate-700"
                                autoFocus
                              />
                                ...
                                {allTechniques
                                  ...
                                  .map(t => (
                                    <button
                                      ...
                                      className="w-full flex items-center gap-2 px-3 py-1.5 hover:bg-slate-800 text-left transition-colors disabled:opacity-50"
                                    >
                          ...
                          {sourceTechs?.length ? (
                            <>
                              <div className="grid grid-cols-2 gap-1.5 max-h-64 overflow-y-auto">
                                {sourceTechs.map(t => (
                                  <div key={t.id} className="flex items-center gap-2 px-2.5 py-1.5 rounded-lg bg-slate-900 border border-slate-800 group">
                                    <span className="font-mono text-xs text-blue-400 flex-shrink-0">{t.id}</span>
                                    <span className="text-xs text-slate-300 truncate flex-1">{t.name}</span>
                                    <span className={`text-xs flex-shrink-0 ${t.has_detection ? 'text-emerald-400' : 'text-slate-600'}`}>
                                      {t.has_detection ? '◉' : '○'}
                                    </span>
                                    <button
                                      onClick={() => removeTechnique(source.id, t.id)}
                                      className="text-slate-600 hover:text-red-400 opacity-0 group-hover:opacity-100 flex-shrink-0 transition-all text-xs leading-none"
                                      title="Remove technique"
                                    >×</button>
                                  </div>
                                ))}
                              </div>
                              <p className="text-xs text-slate-600 mt-2">◉ = has active detection · hover to remove</p>
                            </>
                          ) : (
                            <div className="text-xs text-slate-500 py-2">No techniques mapped. Click "Add Technique" above.</div>
                          )}
                        </div>
                      )}
                    </div>
            ...
            {filtered.length === 0 && (
              <div className="text-center text-slate-500 py-12 text-sm">No data sources match your filters</div>
            )}
          </div>
        )}
      </div>
      ...
      {deletingId !== null && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60">
          <div className="bg-slate-900 border border-slate-700 rounded-xl p-6 w-full max-w-sm shadow-xl">
            <h2 className="text-base font-semibold text-slate-100 mb-2">Delete Data Source</h2>
            <p className="text-sm text-slate-400 mb-4">
              Permanently delete <span className="text-slate-200 font-medium">
