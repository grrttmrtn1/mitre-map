  ...
  return (
    <div className="flex flex-col h-full">
        ...
        <div className="flex items-start justify-between">
          ...
          <div className="flex gap-2">
            <button onClick={takeSnapshot} disabled={snapping}
              className="px-3 py-1.5 text-sm bg-slate-700 text-slate-300 border border-slate-600 rounded-lg hover:bg-slate-600 transition-colors disabled:opacity-50">
            ...
            <div className="relative group">
              ...
              <div className="absolute right-0 top-full mt-1 w-56 bg-slate-800 border border-slate-700 rounded-lg shadow-xl z-50 hidden group-hover:block">
                {exportLinks.map(l => (
                  <a key={l.href} href={l.href} download
                    className="flex items-center gap-2 px-3 py-2.5 text-sm text-slate-300 hover:bg-slate-700 hover:text-white first:rounded-t-lg last:rounded-b-lg transition-colors">
                    {l.label}
                  </a>
                ))}
              </div>
      ...
      <div className="flex-1 overflow-y-auto p-6">
        ...
        {activeTab === 'executive' && (
          loadingExec ? (
            <div className="flex items-center justify-center h-40 text-slate-500">Generating executive report…</div>
          ) : !report ? (
            <div className="text-slate-500 text-center py-12">Failed to load report.</div>
          ) : (
            <div className="space-y-6">
              ...
              {report.trend && (
                <div className={`rounded-xl p-4 border ${report.trend.coverage_change >= 0 ? 'bg-emerald-500/5 border-emerald-500/20' : 'bg-red-500/5 border-red-500/20'}`}>
                  <div className="text-xs font-medium text-slate-400 mb-1">Since last snapshot</div>
              ...
              <div className="bg-slate-900 border border-slate-800 rounded-xl p-4">
                <h2 className="text-sm font-medium text-slate-300 mb-4">Coverage by Tactic</h2>
              ...
              <div className="bg-slate-900 border border-slate-800 rounded-xl p-4">
                <h2 className="text-sm font-medium text-slate-300 mb-3">Top 10 Undetected Techniques</h2>
        ...
        {activeTab === 'trends' && (
          loadingTrends ? (
            <div className="flex items-center justify-center h-40 text-slate-500">Loading trends…</div>
          ) : (
            <div className="space-y-6">
              <div className="bg-slate-900 border border-slate-800 rounded-xl p-4">
                <div className="flex items-center justify-between mb-4">
                  <h2 className="text-sm font-medium text-slate-300">Coverage % Over Time</h2>
              ...
              <div className="bg-slate-900 border border-slate-800 rounded-xl p-4">
                <h2 className="text-sm font-medium text-slate-300 mb-3">Snapshot History</h2>
                  ...
                  <tbody>
                    {[...snapshots].reverse().map(s => (
                      <tr key={s.id} className="border-b border-slate-800/50 hover:bg-slate-800/20">
                        <td className="py-2 px-3 text-slate-400">{new Date(s.taken_at).toLocaleString()}</td>
                        <td className="py-2 px-3">
                          <span className={`font-mono font-bold ${s.coverage_pct >= 50 ? 'text-emerald-400' : s.coverage_pct >= 25 ? 'text-yellow-400' : 'text-red-400'}`}>
                            {s.coverage_pct}%
                          </span>
                        </td>
                        <td className="py-2 px-3 text-slate-400">{s.covered_techniques}/{s.total_techniques}</td>
                        <td className="py-2 px-3 text-red-400">{s.gap_techniques}</td>
                        <td className="py-2 px-3 text-slate-400">{s.active_detections}</td>
                        <td className="py-2 px-3 text-slate-400">{s.total_tools}</td>
                        <td className="py-2 px-3 text-slate-500">{s.notes ?? '—'}</td>
                        <td className="py-2 px-3">
                          <button onClick={() => deleteSnapshot(s.id)} className="text-slate-600 hover:text-red-400">×</button>
                        </td>
                      </tr>
                    ))}
                  </tbody>
        ...
        {activeTab === 'threats' && (
          loadingThreats ? (
            <div className="flex items-center justify-center h-40 text-slate-500">Loading threat landscape…</div>
          ) : threatReport ? (
                ...
                <div key={g.id} className="bg-slate-900 border border-slate-800 rounded-xl p-4">
                  <div className="flex items-start justify-between">
                    ...
                    <div className="text-right">
                      ...
                      <div className="text-xs text-slate-500">exposed</div>
                    </div>
                  </div>
                  <div className="mt-3 grid grid-cols-3 gap-4 text-xs">
                    <div><span className="text-slate-500">Total techniques: </span><span className="text-slate-300">{g.total_techniques}</span></div>
                    <div><span className="text-slate-500">Covered: </span><span className="text-emerald-400">{g.covered}</span></div>
                    <div><span className="text-slate-500">Exposed: </span><span className="text-red-400">{g.exposure}</span></div>
                  </div>
        ...
        {activeTab === 'gaps' && (
          loadingGaps ? (
            <div className="flex items-center justify-center h-40 text-slate-500">Loading gap report…</div>
          ) : gapReport ? (
            <div className="space-y-2">
              <div className="text-xs text-slate-500 mb-4">{gapReport.total_gaps} undetected techniques ranked by priority score</div>
                  ...
                  {gapReport.gaps.slice(0, 60).map((g: any) => (
                    <tr key={g.id} className="border-b border-slate-800/50 hover:bg-slate-800/20">
                      <td className="py-2.5 px-4">
                        ...
                        <div className="text-xs text-slate-300">{g.name}</div>
                      </td>
                      <td className="py-2.5 px-4">
                      ...
                      <td className="py-2.5 px-4">
                        <span className={`font-mono text-xs ${g.group_count >= 3 ? 'text-red-400' : g.group_count >= 1 ? 'text-orange-400' : 'text-slate-600'}`}>
                          {g.group_count}
                        </span>
                      </td>
                      <td className="py-2.5 px-4 text-xs text-slate-400">{g.compliance_impact}</td>
                      <td className="py-2.5 px-4">
                        <span className={`text-xs ${g.mitigated ? 'text-yellow-400' : 'text-slate-600'}`}>
                      ...
                      <td className="py-2.5 px-4">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-xs text-slate-400 font-mono">{g.priority_score}</span>
                        </div>
                      </td>
                    </tr>
                  ))}
