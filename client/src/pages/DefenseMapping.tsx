        ...
        <div className="px-4 py-4 border-b border-slate-800">
          ...
          <div className="mt-2 space-y-2">
            <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search..."
              className="w-full px-3 py-1.5 text-xs bg-slate-800 border border-slate-700 rounded-lg text-slate-300 focus:outline-none focus:border-blue-500" />
          ...
          <div className="p-6 space-y-6">
            <div>
              <div className="flex items-start justify-between">
                <div>
                  <div className="text-xs font-mono text-slate-500">{selected.technique.id}</div>
                  <h1 className="text-xl font-semibold text-slate-100">{selected.technique.name}</h1>
                  <div className="flex flex-wrap gap-1 mt-1">
                    {selected.technique.tactic_ids.map(tid => {
                      const tac = tactics.find(t => t.id === tid);
                      return tac ? (
                        <span key={tid} className="px-2 py-0.5 bg-slate-800 text-slate-400 rounded text-xs">{tac.name}</span>
                      ) : null;
                    })}
                  </div>
                  {selected.technique.description && (
                    <p className="text-sm text-slate-400 mt-2 max-w-2xl">{selected.technique.description}</p>
                  )}
                </div>
            ...
            <div className="grid grid-cols-3 gap-4">
              <div className={`rounded-xl border p-3 ${selected.detections.filter(d => d.status === 'active').length > 0 ? 'border-emerald-500/30 bg-emerald-500/5' : 'border-slate-700 bg-slate-800/30'}`}>
                <div className="text-xs text-slate-400">Active Detections</div>
                <div className={`text-2xl font-bold mt-1 ${selected.detections.filter(d => d.status === 'active').length > 0 ? 'text-emerald-400' : 'text-slate-500'}`}>
                  {selected.detections.filter(d => d.status === 'active').length}
                </div>
              </div>
              <div className={`rounded-xl border p-3 ${selected.d3fend.length > 0 ? 'border-blue-500/30 bg-blue-500/5' : 'border-slate-700 bg-slate-800/30'}`}>
                <div className="text-xs text-slate-400">D3FEND Countermeasures</div>
                <div className={`text-2xl font-bold mt-1 ${selected.d3fend.length > 0 ? 'text-blue-400' : 'text-slate-500'}`}>{selected.d3fend.length}</div>
              </div>
              <div className={`rounded-xl border p-3 ${selected.mitigations.length > 0 ? 'border-purple-500/30 bg-purple-500/5' : 'border-slate-700 bg-slate-800/30'}`}>
                <div className="text-xs text-slate-400">ATT&CK Mitigations</div>
                <div className={`text-2xl font-bold mt-1 ${selected.mitigations.length > 0 ? 'text-purple-400' : 'text-slate-500'}`}>{selected.mitigations.length}</div>
              </div>
            </div>

            <div className="grid grid-cols-2 gap-4">
              <div className="bg-slate-900 border border-slate-800 rounded-xl p-4">
                <h2 className="text-sm font-semibold text-slate-300 mb-3">SIEM Detections</h2>
                {selected.detections.length === 0 ? (
                  <div className="text-xs text-slate-500 italic">No detections for this technique.</div>
                ) : (
                  <div className="space-y-2">
                    {selected.detections.map(d => (
                      <div key={d.id} className="bg-slate-800/60 rounded-lg p-3">
                        <div className="flex items-start justify-between gap-2">
                          <div>
                            <div className="text-xs font-medium text-slate-200">{d.name}</div>
                            {d.rule_id && <div className="font-mono text-xs text-slate-500 mt-0.5">{d.rule_id}</div>}
              ...
              <div className="bg-slate-900 border border-slate-800 rounded-xl p-4">
                <h2 className="text-sm font-semibold text-slate-300 mb-3">D3FEND Countermeasures</h2>
                {selected.d3fend.length === 0 ? (
                  <div className="text-xs text-slate-500 italic">No D3FEND countermeasures mapped.</div>
                ) : (
                  <div className="space-y-2">
                    {Object.entries(
                      ...
                      <div key={cat}>
                        <span className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-medium border ${D3FEND_CATEGORY_COLORS[cat] ?? 'bg-slate-700 text-slate-400 border-slate-600'} mb-1`}>{cat}</span>
                        {items.map(d => (
                          <div key={d.id} className="bg-slate-800/60 rounded-lg p-2.5 mb-1.5">
                            <div className="flex items-start gap-2">
                              <span className="font-mono text-xs text-slate-500 w-14 flex-shrink-0 mt-0.5">{d.id}</span>
                              <div>
                                <div className="text-xs font-medium text-slate-200">{d.name}</div>
                                <div className="text-xs text-slate-500 mt-0.5">{d.subcategory}</div>
                              </div>
                            </div>
                          </div>
                        ))}
            ...
            <div className="bg-slate-900 border border-slate-800 rounded-xl p-4">
              <h2 className="text-sm font-semibold text-slate-300 mb-3">ATT&CK Mitigations</h2>
              {selected.mitigations.length === 0 ? (
                <div className="text-xs text-slate-500 italic">No ATT&CK mitigations mapped.</div>
              ) : (
